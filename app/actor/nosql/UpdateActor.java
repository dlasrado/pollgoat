	...
	public void onReceive(Object request) throws Exception {
		...
		if(request instanceof FormParamBean){
        	...
        	try {
				...
				Promise.wrap(MongoUtil.update(MongoUtil.connect(connection,al.get(1),requestBean.getCollectionName()),requestBean.getCriteria(),requestBean.getData())).map(new Function<LastError,Result>(){
					...
					public Result apply(LastError error)
						...
						connection.close();
						if(!error.inError())
							sender.tell("record updated successfully",getSelf());
						else{
