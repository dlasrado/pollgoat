	...
	public void onReceive(Object request) throws Exception {
		...
		if(request instanceof FormParamBean){
        	...
        	try {
                ...
                Promise.wrap(MongoUtil.delete(MongoUtil.connect(connection,al.get(1),requestBean.getCollectionName()),requestBean.getCriteria())).map(new Function<LastError,Result>(){
						...
						public Result apply(LastError error)
							...
							connection.close();
							if(!error.inError())
								sender.tell("record deleted successfully",getSelf());
							else{
